<div x-data="{modalOpen : false, unit_id: null, unit_name :''}"
    ...
    <p class="text-lg">Unit List</p>

    @if (auth('web')->user()->hasPermission("units", 'create'))
    <a href="{{route('units.create')}}" wire:navigate class="py-1 px-4 border rounded bg-blue-700 hover:bg-blue-900 ml-auto text-white block w-fit cursor-pointer">
        Create Unit
    </a>
    @endif
    @if($units->count())
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="p-4 border-b border-blue-gray-50 w-48">
                        ...
                        <button @click="modalOpen = true;unit_id='{{$unit->id}}'; unit_name=`{{$unit->name}}`"
                            class="btn_unit_delete inline-block font-sans text-sm antialiased font-medium leading-normal text-blue-gray-900">
                            Delete
                        </button>
                        @endif
                    </td>
    ...
    @else
    <h2 class="text-3xl text-center">No unit yet!</h2>
    @endif
    <x-modal feature="unit" />
</div>